...
function QueueProvider({ children }: PropsWithChildren) {
  ...
  const fetchQueueSnapshot = useCallback(async (activeEventId: string) => {
    ...
    const [eventData, tipMessages, eventTypeSettings, audienceLocaleSettings, venueLogoLayoutSettings, mirrorQrSettings] = await Promise.all([
      withTimeout(
        loadEventSnapshot(),
        DEFAULT_DB_TIMEOUT_MS,
