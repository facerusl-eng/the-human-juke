...
const DEGRADED_AUDIENCE_LIVE_DISCOVERY_POLL_INTERVAL_MS = 4_000
...
function isMissingMirrorQrSettingsColumnError(error: unknown) {
  if (!error || typeof error !== 'object') {
    return false
  }
...
function isMissingMirrorQrFlashColumnError(error: unknown) {
  if (!error || typeof error !== 'object') {
    return false
  }
...
function QueueProvider({ children }: PropsWithChildren) {
  ...
  useEffect(() => {
    ...
    // Prevent hydration if snapshot matches current state (avoid infinite update loop)
    const isSameState =
      JSON.stringify(snapshot.event) === JSON.stringify(event) &&
      JSON.stringify(snapshot.hostEvents) === JSON.stringify(hostEvents) &&
      JSON.stringify(snapshot.songs) === JSON.stringify(songs) &&
      JSON.stringify(snapshot.performedSongs) === JSON.stringify(performedSongs)
  ...
  const fetchQueueSnapshot = useCallback(async (activeEventId: string) => {
    ...
    const loadMirrorQrSettings = async (): Promise<MirrorQrSettings> => {
      try {
        ...
        if (error) {
          if (isMissingMirrorQrSettingsColumnError(error)) {
            hasMirrorQrSettingsColumns = false
    ...
    const loadMirrorQrFlashSettings = async (): Promise<MirrorQrFlashSettings> => {
      try {
        ...
        return {
          ...
          mirror_brb_qr_flash_venue: (row.mirror_brb_qr_flash_venue as string | null) ?? null,
        }
      } catch {
        return { mirror_brb_qr_flash_enabled: true, mirror_brb_qr_flash_venue: null }
      }
  ...
  useEffect(() => {
    ...
    void idbGetPendingSongs(targetEventId).then(async (pending) => {
      ...
      if (isLastSongSoonPlaybackState(sharedPlaybackState)) {
        return
      }
  ...
  const value = useMemo(
    () => ({
      ...
      updateEventSettings: async (updates: EventSettingsUpdates) => {
        ...
        if (Array.isArray(updates.selectedPlaylistIds)) {
