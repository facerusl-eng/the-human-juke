...
function getSpotifyRedirectUri(req) {
  if (process.env.NODE_ENV !== 'production') {
    ...
    if (chosenHost) {
      ...
      const protocol = typeof protocolHeader === 'string' && protocolHeader.length > 0
        ? protocolHeader.split(',')[0].trim()
        : 'http'

      // Vite proxy can point API requests at localhost:3001; map callback back to the app dev port.
      const frontendHost = chosenHost.replace(/:3001$/, ':5173')
      return `${protocol}://${frontendHost}/callback`
    }
