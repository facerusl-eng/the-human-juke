...
// Utility to parse .lab chord file
function parseChordLab(content: string): ChordSync[] {
  ...
  return lines
    .map((line, idx) => {
      ...
      if (parts.length >= 3) {
        ...
        return { id: `chord-${idx}`, symbol, timeSec }
      }
      return null
    })
    .filter(Boolean) as ChordSync[]
}

// Utility to parse Whisper .txt (one line per phrase)
function parseLyricsTxt(content: string): LyricLine[] {
  return content
    .split(/\r?\n/)
    .filter((line) => line.trim().length > 0)
    .map((text, idx) => ({
      id: `lyric-${idx}`,
      startSec: idx * 3, // Placeholder: 3s per line, real sync needs Whisper JSON
      endSec: (idx + 1) * 3,
      words: [{ id: `w-${idx}`, text, startSec: idx * 3, endSec: (idx + 1) * 3 }],
      chords: [],
    }))
}
...
function buildChartFromLyrics(rawLyrics: string, durationSec: number): GeneratedChart {
  ...
  const lines = lyricLines.map((lineText, lineIndex) => {
    ...
    // Assign a chord to each word for karaoke sync style
    const chords: ChordSync[] = words.map((word, idx) => ({
      id: createId('chord'),
      symbol: chordPool[(lineIndex + idx) % chordPool.length],
  ...
  return {
    lines,
    statusMessage: `Auto lyrics extracted (${lyricLines.length} lines). Chords are now synced to each word. Review sync and chords in Edit Mode.`,
  }
}
...
async function requestAutoChart(primaryTrack: StudioTrack | null): Promise<GeneratedChart> {
  ...
  try {
    // Send both audio and lyrics as FormData
    const formData = new FormData();
    formData.append('audio', primaryTrack.file, primaryTrack.name);
    // Use manualLyricsInput if present, else empty string
    // @ts-ignore - manualLyricsInput is in closure
    formData.append('lyrics', manualLyricsInput || '');
...
function SongStudioPage() {
  ...
  // Track if lyrics have been entered (manual or file)
  // const [lyricsReady, setLyricsReady] = useState(false)
  const [shouldPromptLyrics, setShouldPromptLyrics] = useState(false)
  ...
  const onAddSongFile = async (fileList: FileList | null) => {
    ...
    // If no lyrics are present, prompt for lyrics
    setShouldPromptLyrics(true)
    // Focus lyrics textarea when prompting for lyrics
    useEffect(() => {
      if (shouldPromptLyrics && lyricsInputRef.current) {
    ...
    <section className="surface-card page-shell" aria-label="Song studio page">
      ...
      <article className="studio-panel">
        <div className="studio-upload-row">
            <label className="studio-upload-btn" htmlFor="studio-bulk-import">Bulk Import (audio + lyrics + chords)</label>
            <input
              id="studio-bulk-import"
              type="file"
              multiple
              accept="audio/*,.mp3,.wav,.aac,.m4a,.ogg,.txt,.lab"
              onChange={async (event) => {
                const files = Array.from(event.target.files || [])
                let audioFile: File | null = null
                let lyricsFile: File | null = null
                let chordsFile: File | null = null
                console.log('[BulkImport] Selected files:', files.map(f => f.name))
                for (const file of files) {
                  if (file.name.match(/\.(mp3|wav|aac|m4a|ogg)$/i)) audioFile = file
                  else if (file.name.match(/\.txt$/i)) lyricsFile = file
                  else if (file.name.match(/\.lab$/i)) chordsFile = file
                }
                console.log('[BulkImport] Detected audio:', audioFile?.name, 'lyrics:', lyricsFile?.name, 'chords:', chordsFile?.name)
                if (!audioFile) {
                  alert('Please include an audio file.')
                  return
                }
                try {
                  // Patch: onAddSongFile expects FileList, not File[]
                  const fileList = {
                    0: audioFile,
                    length: 1,
                    item: (index: number) => (index === 0 ? audioFile : null)
                  } as unknown as FileList;
                  await onAddSongFile(fileList)
                  console.log('[BulkImport] Audio loaded')
                } catch (err) {
                  console.error('[BulkImport] Error loading audio:', err)
                }
                // Load lyrics
                ...
                if (lyricsFile) {
                  try {
                    ...
                    const lines = parseLyricsTxt(txt)
                    setChartLines(lines)
                    // lyricsLoaded = true
                    // setLyricsReady(true)
                    console.log('[BulkImport] Lyrics loaded')
                  } catch (err) {
                    console.error('[BulkImport] Error loading lyrics:', err)
                  }
                }
                // Load chords
                if (chordsFile) {
                  try {
                    const lab = await chordsFile.text()
                    const chords = parseChordLab(lab)
                    setChartLines((prev) => prev.map((line) => ({
                      ...line,
                      chords: chords.filter((c) => c.timeSec >= line.startSec && c.timeSec < line.endSec),
                    })))
                    console.log('[BulkImport] Chords loaded')
                  } catch (err) {
                    console.error('[BulkImport] Error loading chords:', err)
                  }
                }
                // Only reset input if it exists
        ...
        <div className="studio-manual-lyrics">
          ...
          {shouldPromptLyrics && (
            <div className="studio-lyrics-prompt" style={{ color: 'red', fontWeight: 'bold', marginBottom: 8 }}>
              Please paste or type the lyrics for your song below, then click "Apply Manual Lyrics".
            </div>
          )}
          <textarea
            ...
            style={shouldPromptLyrics ? { border: '2px solid red', background: '#fffbe6' } : {}}
          />
          <button type="button" onClick={() => { onApplyManualLyrics(); setShouldPromptLyrics(false); }}>Apply Manual Lyrics</button>
        </div>
        <div className="studio-bulk-import-help">
          <p><b>Bulk Import:</b> Select your audio, .txt (lyrics), and .lab (chords) files together. The app will auto-load and sync them for playback and editing.</p>
        </div>
      </article>
